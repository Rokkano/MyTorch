...
void Tensor<T, B>::plot(TensorSpan tensors, const std::string &linespec, OpenCVWindowOpts opts)
    ...
    for (std::size_t i = 0; i < tensors.size(); i++)
    {
        if (tensors[i].get().shape().size() != 1)
            throw TensorInvalidShapeException(
                std::format("Shape {} is invalid for plot : need a single dimentional tensor.",
                            Tensor<T, B>::shapeToStr(tensors[i].get().shape())));
        if (tensors[i].get().shape()[0] != len)
            throw TensorInvalidShapeException(std::format("Plot tensors must all have the same length."));
