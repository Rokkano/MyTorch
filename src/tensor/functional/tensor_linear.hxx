    ...
    requires std::is_arithmetic_v<T>
{
    if (lhs.shape_.size() != 1)
        throw TensorInvalidShapeException(std::format("Dot product only applies for 1-dimensional tensors : {}.",
                                                      Tensor<T>::tensorShapeToStr(lhs.shape_)));
    if (rhs.shape_.size() != 1)
        throw TensorInvalidShapeException(std::format("Dot product only applies for 1-dimensional tensors : {}.",
                                                      Tensor<T>::tensorShapeToStr(rhs.shape_)));
    ...
    requires std::is_arithmetic_v<T>
{
    ...
    if (lhs.shape_.size() != 2)
        throw TensorInvalidShapeException(
            std::format("mm only applies for 2-dimensional tensors : {}.", Tensor<T>::tensorShapeToStr(lhs.shape_)));
    if (rhs.shape_.size() != 2)
        throw TensorInvalidShapeException(
            std::format("mm only applies for 2-dimensional tensors : {}.", Tensor<T>::tensorShapeToStr(rhs.shape_)));
    if (lhs.shape_[1] != rhs.shape_[0])
        throw TensorInvalidShapeException(
            std::format("Tensors are not compatible for matrix multiplication : {} and {}.",
                        Tensor<T>::tensorShapeToStr(lhs.shape_), Tensor<T>::tensorShapeToStr(rhs.shape_)));
