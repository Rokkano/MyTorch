...
{
    Tensor<U> *tensor = new Tensor<U>(this->shape_);
...
template <typename T>
Tensor<T> *Tensor<T>::from_function(std::function<size_t(T)> lambda, std::vector<std::size_t> shape)
{
...
template <typename T>
Tensor<T> *Tensor<T>::from_vector(std::vector<T> buffer, std::vector<std::size_t> shape)
{
