...
T MTFile<T>::read(std::string path) 
{
    ...
    if (mtFileVersionMajor != MTFILE_VERSION_MAJOR || mtFileVersionMinor != MTFILE_VERSION_MINOR)
        throw std::invalid_argument(std::format("MTFILE_VERSION mismatch : expected {}.{}, got {}.{}", MTFILE_VERSION_MAJOR, MTFILE_VERSION_MINOR, mtFileVersionMajor, mtFileVersionMinor));
